            ...
            <form action="{{ route('fasilitas.update', $fasilitas->id) }}" method="POST">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="jenis_fasilitas" name="jenis_fasilitas" required>
                        ...
                        <option value="Tempat Ibadah" {{ old('jenis_fasilitas', $fasilitas->jenis_fasilitas) == 'Tempat Ibadah' ? 'selected' : '' }}>Tempat Ibadah</option>
                        <option value="Pendidikan" {{ old('jenis_fasilitas', $fasilitas->jenis_fasilitas) == 'Pendidikan' ? 'selected' : '' }}>Pendidikan</option>
                        <option value="Kesehatan" {{ old('jenis_fasilitas', $fasilitas->jenis_fasilitas) == 'Kesehatan' ? 'selected' : '' }}>Kesehatan</option>
                        <option value="Umum" {{ old('jenis_fasilitas', $fasilitas->jenis_fasilitas) == 'Umum' ? 'selected' : '' }}>Umum</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="alamat">Alamat</label>
                    <textarea class="form-control" id="alamat" name="alamat" rows="3">{{ old('alamat', $fasilitas->alamat) }}</textarea>
                </div>

                <div class="form-group">
                    <label for="keterangan">Keterangan</label>
                    <textarea class="form-control" id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $fasilitas->keterangan) }}</textarea>
                </div>
