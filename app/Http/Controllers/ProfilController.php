...
class ProfilController extends Controller
{
    public function index(Request $request)
    {
        ...
        $lampidData = (clone $pendudukQuery)->select('lampid', DB::raw('count(*) as total'))
            ->whereNotNull('lampid')->groupBy('lampid')->pluck('total', 'lampid');
        $lampid = [
            'kelahiran' => $lampidData['kelahiran'] ?? 0,
            'kematian'  => $lampidData['kematian'] ?? 0,
            'pindah'    => $lampidData['pindah'] ?? 0,
            'datang'    => $lampidData['datang'] ?? 0,
        ];
        ...
        $currentYear = date('Y');
        $kelompokUsia = [
            'Balita' => (clone $pendudukQuery)->whereRaw("$currentYear - year BETWEEN 0 AND 5")->count(),
            'Anak'   => (clone $pendudukQuery)->whereRaw("$currentYear - year BETWEEN 6 AND 12")->count(),
            'Remaja' => (clone $pendudukQuery)->whereRaw("$currentYear - year BETWEEN 13 AND 18")->count(),
            'Dewasa' => (clone $pendudukQuery)->whereRaw("$currentYear - year BETWEEN 19 AND 55")->count(),
            'Lansia' => (clone $pendudukQuery)->whereRaw("$currentYear - year >= 56")->count(),
        ];
